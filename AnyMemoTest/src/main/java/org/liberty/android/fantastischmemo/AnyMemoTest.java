...
public class AnyMemoTest extends ActivityInstrumentationTestCase2<Activity>{
    ...
    public void test() {
        ...
        Intents.init();
        loopRunnable.startInstrument = true;

        onView(withText("Download")).perform(click());
        onView(withText("anymemo.org\n(default)")).perform(click());
        mDevice.setOrientationRight();
        mDevice.setOrientationNatural();
        onData(anything()).inAdapterView(
            withId("org.liberty.android.fantastischmemo:id/file_list")
            ).atPosition(0).check(matches(withText("Geography")));
